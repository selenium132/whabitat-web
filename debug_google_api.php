...
try {
    ...
    // Step 3: Try creating a spreadsheet
    echo "<h2>3. Create Test</h2>";
    try {
        $sheet = $gs->createSpreadsheet('TEST_DELETE_ME_' . time());
        echo "<p style='color:green;'>✓ CREATE works! Sheet ID: " . $sheet['spreadsheetId'] . "</p>";
        echo "<p><a href='https://docs.google.com/spreadsheets/d/" . $sheet['spreadsheetId'] . "' target='_blank'>Open Sheet</a></p>";
    } catch (Exception $e) {
        echo "<p style='color:red;'>✗ CREATE failed</p>";

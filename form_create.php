...
<body>
    ...
    <form id="mainForm" method="POST" action="">
        ...
        <div class="form-builder-container">
                ...
                <div class="meta-info">
                    ...
                    <div class="admin-selection-area">
                        <details class="admin-selection-details">
                            <summary>
                                <span><i class="fas fa-user-shield"></i> 管理者</span>
                            </summary>
                    ...
                    <div class="admin-selection-area">
                        <details class="admin-selection-details">
                            <summary>
                                <span><i class="fas fa-users"></i> 対象者 <small style="color:#888;">(未選択=全員)</small></span>
                            </summary>
                                    ...
                                    <div>
                                        ...
                                        <div style="color: #888; font-size: 0.85rem; margin-top: 4px; line-height: 1.6;">
                                            <?php if ($is_survey_form): ?>
                                                OFFのとき、一般会員に見えるのは回答者数だけです（誰が回答したかは分かりません）。<br>
                                                ONにすると、回答した人の一覧と、質問ごとの「公開」がONの回答が見られます。
                                            <?php else: ?>
                                                OFFにすると、一般会員に見えるのは人数（参加◯名／不参加◯名／未定◯名）だけになり、<br>
                                                誰が参加するかは管理者だけが見られます。
                                            <?php endif; ?>
                                        </div>
                                    </div>
            ...
            <!-- Attendance Question Notice (Events only) -->
            <?php if ($type !== 'survey'): ?>
    ...
    <script>
        ...
        const existingSchema = <?php 
            ...
            echo json_encode($schema_data, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP);
        ?>;
        ...
        function togglePublic(id) {
            const icon = document.getElementById(`pub-toggle-${id}`);
            const isPub = icon.getAttribute('data-public') === 'true';
            ...
            if (!isPub) {
                ...
                icon.className = 'fas fa-toggle-off';
                icon.style.color = '#ccc';
                icon.setAttribute('data-public', 'false');
            }
        }
        ...
        function saveForm() {
            ...
            const isSurvey = typeInput && typeInput.value === 'survey';
            
            // Only check event_date for events, not surveys
            if (!isSurvey && eventDateInput && !eventDateInput.value) {
                alert("開催日時を設定してください");
                return;
            }
            ...
            cards.forEach(card => {
                ...
                const required = card.querySelector('#req-toggle-' + id)?.getAttribute('data-required') === 'true';
                const isPublic = card.querySelector('#pub-toggle-' + id)?.getAttribute('data-public') === 'true';
    ...
    <?php if ($edit_mode): ?>
    <form id="deleteForm" method="POST" action="form_delete.php" style="display: none;">
        <input type="hidden" name="csrf_token" value="<?php echo generateCsrfToken(); ?>">
